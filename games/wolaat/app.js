...
(function () {
  ...
  function paintSetup() {
    ...
    $('setup-hint').textContent = cfg.mode === 'solo'
      ...
      : (cfg.mode === 'bt'
        ? 'البلوتوث: شغّله في كل الأجهزة وقرّبها. عند فتح الغرفة يظهر جهازك للاعبين، وينضمون من «انضم كلاعب ← بالبلوتوث».'
  ...
  $('b-btjoin').onclick = function () {
    const AJ = window.AbuJanan;
    if (!AJ || !AJ.bt.available()) { toast('اللعب بالبلوتوث متاح في نسخة التطبيق (APK) فقط.'); return; }
    const name = ($('j-name').value || '').trim();
    if (!name) { toast('اكتب اسمك أولًا.'); $('j-name').focus(); return; }
    const btn = this;
    AJ.bt.pick({ title: 'اختر جهاز الحكم' }).then((dev) => {
      btn.disabled = true; $('j-status').textContent = 'جاري الاتصال بـ ' + dev.name + '…';
      role = 'player'; S = null; myPid = null;
      return tryJoin('bt', dev.id, name).then(() => { btn.disabled = false; });
    }).catch((e) => { btn.disabled = false; if (e && e.message === 'cancelled') return; $('j-status').textContent = (e && e.message) || 'ما قدرنا ندخل الغرفة.'; });
  };
      ...
      NET.Client(mode, code, cid, {
        ...
        onMsg: (m) => {
          if (m.t === 'welcome') {
            ...
            res(); toast('دخلت الغرفة ' + (mode === 'bt' ? '📶' : code));
          } else if (m.t === 'state') {
  ...
  function renderLobby() {
    ...
    $('lb-mode').textContent = { solo: '📱 جهاز واحد', local: '🖥️ نوافذ متعددة', online: '🌐 أجهزة مختلفة', bt: '📶 بلوتوث' }[S.mode] || '';
    $('lb-help').innerHTML = S.mode === 'solo'
      ...
      : (S.mode === 'local'
        ...
        : (S.mode === 'bt'
          ? 'كل لاعب يفتح ولعت على جواله ← «انضم كلاعب» ← «📶 انضم بالبلوتوث» ← يختار هذا الجهاز من القائمة. بلا إنترنت.'
  ...
  renderHome();
  /* زر الرجوع في أندرويد + زر الرئيسية من الطبقة المشتركة */
  if (window.AbuJanan) {
    window.AbuJanan.addHomeButton({ pos: 'bl' });
    window.AbuJanan.onBack = () => {
      const v = document.querySelector('.view.active'); const id = v ? v.id : 'v-home';
      if (id === 'v-home') return false;
      if (id === 'v-judge' || id === 'v-play' || id === 'v-lobby') { $('b-leave') && $('b-leave').click && (id === 'v-lobby' ? $('b-leave').click() : toast('للخروج استخدم زر الخروج في الأعلى')); return true; }
      show('v-home'); renderHome && renderHome(); return true;
    };
  }
