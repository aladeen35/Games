/* ============================================================================
   ...
   ثلاثة أوضاع بنفس البروتوكول:
     ...
     bt     — أجهزة قريبة عبر البلوتوث بلا إنترنت (نسخة التطبيق، عبر AbuJanan.bt).
   ============================================================================ */
(function (root) {
  'use strict';
  ...
  function Host(mode, code, h) {
    ...
    let bt = null;              /* اتصال البلوتوث (المضيف) */
    function deliver(m) {
      if (!m || m.to !== 'host') return;
      if (mode !== 'bt' && m.room !== code) return;
    ...
    const api = {
      ...
      send(cid, msg) {
        ...
        if (mode === 'local' && bus) bus.post(m);
          ...
          if (cid) { const c = conns.get(cid); bt.send(m, c && c.dev ? c.dev : undefined); }
          else bt.send(m);
        }
        else if (mode === 'online') {
      ...
      close() {
        ...
        if (bus) bus.close();
        if (bt) try { bt.close(); } catch (_) {}
        if (peer) try { peer.destroy(); } catch (_) {}
      },
    };

    if (mode === 'bt') {
      const AJ = root.AbuJanan;
      if (!AJ || !AJ.bt.available()) return Promise.reject(new Error('اللعب بالبلوتوث متاح في نسخة التطبيق (APK) فقط.'));
      h.onStatus('wait', 'جاري تشغيل البلوتوث…');
      const devOf = new Map();  /* cid → عنوان الجهاز */
      return AJ.bt.host({
        onData: (m, from) => {
          if (m && m.cid) { devOf.set(m.cid, from); const c = conns.get(m.cid); if (c) c.dev = from; }
          if (m && m.t === 'hello') conns.set(m.cid, { open: true, dev: from, send: () => {} });
          deliver(m);
        },
        onJoin: (id, name) => h.onStatus('ready', 'اتصل جهاز: ' + name),
        onLeave: (id) => { conns.forEach((c, cid) => { if (c.dev === id) { conns.delete(cid); h.onLeave(cid); } }); },
        onError: (e) => h.onStatus('warn', e),
      }).then((link) => {
        bt = link;
        api.note = (cid) => { conns.set(cid, conns.get(cid) || { open: true, dev: devOf.get(cid), send: () => {} }); };
        h.onStatus('ready', 'الغرفة مفتوحة بالبلوتوث باسم «' + (link.name || 'جهازي') + '» — اللاعبون يختارون هذا الجهاز من البحث.');
        return api;
      });
    }
  ...
  function Client(mode, code, cid, h) {
    ...
    let bus = null, peer = null, conn = null, closed = false, bt = null;

    function deliver(m) {
      if (!m || m.from !== 'host') return;
      if (mode === 'bt') { if (m.t === 'welcome' && m.code) code = api.code = m.code; }
      else if (m.room !== code) return;
    ...
    if (mode === 'bt') {
      const AJ = root.AbuJanan;
      if (!AJ || !AJ.bt.available()) return Promise.reject(new Error('اللعب بالبلوتوث متاح في نسخة التطبيق (APK) فقط.'));
      /* code هنا = عنوان جهاز المضيف المختار من نافذة البحث */
      h.onStatus('wait', 'جاري الاتصال بالمضيف عبر البلوتوث…');
      return AJ.bt.connect(code, {
        onData: (m) => deliver(m),
        onLeave: () => { if (!closed) h.onStatus('warn', 'انقطع الاتصال بالحكم.'); },
        onError: (e) => h.onStatus('warn', e),
      }).then((link) => { bt = link; h.onStatus('ready', 'متصل بالحكم عبر البلوتوث.'); return api; });
    }
