...
import {
  DIFFICULTIES, isDifficulty, getPhrase, pickPhraseIndex, clamp, computeWpm,
  weekStartUTC, MAX_WPM, ROOM_CODE_ALPHABET, ROOM_CODE_LENGTH,
...
/** أقصر زمن ممكن منطقيًا لكتابة الجملة. */
function minPlausibleMs(phraseLength) {
  return ((phraseLength / 5) / MAX_WPM) * 60000;
}

/** يتحقق من زمن الإكمال ويحسب WPM على الخادم بدل الاعتماد على الواجهة. */
function verifyFinish({ phrase, clientElapsedMs, serverElapsedMs }) {
...
function insertResult(db, { user, difficulty, phraseIndex, wpm, accuracy, roomId = null, now }) {
  db.prepare(`INSERT INTO game_results (userId, displayName, difficulty, phraseIndex, wpm, accuracy, roomId, completedAt)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(user.id, user.displayName, difficulty, phraseIndex, clamp(wpm, 0, 999), clamp(accuracy, 0, 100), roomId, now);
}
...
function maintainRoom(db, room, now) {
  ...
  if (room.status === 'racing') {
    const limit = room.startedAt + DIFFICULTIES[room.difficulty].seconds * 1000 + ROUND_GRACE_MS;
...
function roomView(db, room, user, now) {
  ...
  return {
    room: {
      ...
      seconds: DIFFICULTIES[room.difficulty].seconds,
    },
    serverNow: now,
...
export function createHandlers({ db, presence }) {
  return {
    ...
    'auth.register': (ctx) => {
      ...
      ctx.setSession(createSession(db, user.id, ctx.now));
      return { user };
    },
    ...
    'auth.login': (ctx) => {
      ...
      ctx.setSession(createSession(db, row.id, ctx.now));
    ...
    'room.create': (ctx) => {
      ...
      return tx(db, () => {
        ...
        const phraseIndex = pickPhraseIndex(difficulty);
        const { lastInsertRowid } = db.prepare(
          'INSERT INTO game_rooms (code, difficulty, phraseIndex, createdBy, createdAt) VALUES (?, ?, ?, ?, ?)',
        ).run(code, difficulty, phraseIndex, user.id, ctx.now);
    ...
    'room.join': (ctx) => {
      ...
      return tx(db, () => {
        ...
        if (!existing) {
          ...
          if (n >= MAX_PLAYERS) throw new ApiError(409, 'roomFull', 'الغرفة مكتملة (8 لاعبين)');
        }
        addPlayer(db, room, user, ctx.now);
        return roomView(db, room, user, ctx.now);
      });
    ...
    'room.start': (ctx) => {
      ...
      return tx(db, () => {
        ...
        db.prepare("UPDATE game_rooms SET status = 'racing', startedAt = ? WHERE id = ?").run(ctx.now, room.id);
        db.prepare('UPDATE room_players SET progress = 0, wpm = 0, accuracy = 100, finishedAt = NULL WHERE roomId = ?').run(room.id);
        room.status = 'racing';
        room.startedAt = ctx.now;
        touch(db, room, user.id, ctx.now);
        return roomView(db, room, user, ctx.now);
      });
    },
    ...
    'room.progress': (ctx) => {
      ...
      return tx(db, () => {
        ...
        maintainRoom(db, room, ctx.now);
        if (room.status === 'racing' && me.finishedAt == null) {
          ...
          const len = [...getPhrase(room.difficulty, room.phraseIndex)].length;
    ...
    'room.finish': (ctx) => {
      ...
      return tx(db, () => {
        ...
        if (room.status !== 'racing') fail(409, 'notRacing');
        const wpm = verifyFinish({
          phrase: getPhrase(room.difficulty, room.phraseIndex),
          clientElapsedMs: ctx.body.elapsedMs,
          serverElapsedMs: ctx.now - room.startedAt,
        });
        const accuracy = toInt(ctx.body.accuracy ?? 100, 0, 100);
        ...
        insertResult(db, { user, difficulty: room.difficulty, phraseIndex: room.phraseIndex, wpm, accuracy, roomId: room.id, now: ctx.now });
    ...
    'round.start': (ctx) => {
      const user = requireUser(ctx);
      const difficulty = requireDifficulty(ctx.body.difficulty);
      const n = DIFFICULTIES[difficulty].phrases.length;
      const phraseIndex = toInt(ctx.body.phraseIndex ?? 0, 0, n - 1);
      const id = randomBytes(12).toString('base64url');
      db.prepare('DELETE FROM solo_rounds WHERE startedAt < ?').run(ctx.now - 3600_000);
      db.prepare('INSERT INTO solo_rounds (id, userId, difficulty, phraseIndex, startedAt) VALUES (?, ?, ?, ?, ?)')
        .run(id, user.id, difficulty, phraseIndex, ctx.now);
    ...
    'round.finish': (ctx) => {
      ...
      return tx(db, () => {
        ...
        const limitMs = DIFFICULTIES[round.difficulty].seconds * 1000;
        const wpm = verifyFinish({
          phrase: getPhrase(round.difficulty, round.phraseIndex),
        ...
        insertResult(db, { user, difficulty: round.difficulty, phraseIndex: round.phraseIndex, wpm, accuracy, now: ctx.now });
        return { saved: true, wpm, accuracy };
      });
    },

    // لوحة الترتيب: أفضل نتيجة لكل لاعب هذا الأسبوع، أعلى عشر
    'leaderboard.weekly': (ctx) => {
      const since = weekStartUTC(new Date(ctx.now)).getTime();
      const difficulty = ctx.body.difficulty && ctx.body.difficulty !== 'all' ? requireDifficulty(ctx.body.difficulty) : null;
      const rows = db.prepare(`
        WITH ranked AS (
          ...
          FROM game_results r
          WHERE r.completedAt >= ? AND (? IS NULL OR r.difficulty = ?)
        )
        SELECT userId, displayName, wpm, accuracy, difficulty, completedAt FROM ranked
        WHERE rn = 1
        ORDER BY wpm DESC, accuracy DESC, completedAt ASC
        LIMIT 10`).all(since, difficulty, difficulty);
      return {
        weekStart: since,
